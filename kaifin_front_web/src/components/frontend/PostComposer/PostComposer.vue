<template>
  ...
  <div class="composer">
    ...
    <textarea v-if="!showCode" ref="textareaRef" v-model="postText" class="post-text" placeholder="Whate your mind for posts?" rows="1" @input="autoGrow" >
    </textarea>
  ...
  <div class="code-wrap" v-if="showCode">
    ...
    <div class="group-choose-wrap" v-if="!lockedCommunity">
    ...
    <div class="action-bar">
      <div class="action-left">
        <div class="chip-wrap" ref="pickerRef" v-click-outside-emoji="() => (showEmoji = false)">
    ...
    <div class="picker-body">
      <template v-if="pickerTab === 'stickers'">
        <template v-if="stickerFilterTab === 'all'">
          ...
          <div class="picker-empty" v-else>No sticker Sticker...</div>
        </template>
        ...
        <template v-else-if="stickerFilterTab === 'mine'">
          <div class="sticker-set-list" v-if="mySetsStickers.length">
            <div class="sticker-set-row" v-for="set in mySetsStickers" :key="set.id">
              ...
              <button type="button" class="set-delete-btn" @click.stop="deleteStickerSet(set.id)">Delete Set</button>
            </div>
          </div>
          <div class="picker-empty" v-else>No sticker set...</div>
        </template>
        ...
        <template v-else-if="stickerFilterTab === 'animated'">
          ...
          <div class="picker-empty" v-else-if="!showPackStickers">No sticker pack...</div>
          <template v-else>
        ...
        <template v-else-if="stickerFilterTab === 'create'">
          <div class="create-sticker-panel">
            ...
            <button type="button" class="add-photo-btn" @click.stop="addStickerPack">Add Stickers</button>
...
const props = defineProps({
  lockedCommunity: { type: Object, default: null }
})
...
const groups = ref([])
const selectedGroup = ref(props.lockedCommunity || null)

watch(() => props.lockedCommunity, (val) => {
  if (val) selectedGroup.value = val
})
...
async function submitPost() {
     console.log('images.value before submit:', images.value.length, images.value)
...
topics.value.forEach(tag => {
  ...
  try {
    ...
    selectedGroup.value = props.lockedCommunity || null
...
const vClickOutsideEmoji = {
  mounted(el, binding) {
    el.__clickOutsideEmojiHandler = (e) => {
      if (!el.contains(e.target)) binding.value()
    }
    document.addEventListener('click', el.__clickOutsideEmojiHandler)
  },
  unmounted(el) {
    document.removeEventListener('click', el.__clickOutsideEmojiHandler)
  },
}
</script>
