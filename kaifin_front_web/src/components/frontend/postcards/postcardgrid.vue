    ...
    {{ post.showFullText ? ' See less' : ' See more' }}
      ...
      >
        <div
          ...
          :key="i"
          @click.stop.prevent="togglePhotosExpand(post)"
        >
...
function addOptimisticPost(post) {
  console.log('📊 Before unshift, posts.length:', posts.value.length)
  posts.value.unshift(post)
    console.log('📊 After unshift, posts.value:', posts.value)
  console.log('📊 posts.length now:', posts.value.length)
}

function onNewPost(post) {
   console.log('📥 onNewPost FULL:', JSON.stringify(post, null, 2))
