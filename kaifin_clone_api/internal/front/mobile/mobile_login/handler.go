...
type AuthUserMobileHandlerImpl struct {
	Se *AuthUserMobileServiceImpl
	db *sqlx.DB
	ws *websocket.WebSocketManager
}

func NewAuthUserHandlerImpl(db *sqlx.DB, rdb *redis.Client, ws *websocket.WebSocketManager) *AuthUserMobileHandlerImpl {
	s := NewAuthUserServiceImpl(db, rdb)
	return &AuthUserMobileHandlerImpl{
		Se: s,
		ws: ws,
	}
}
...
// User login
func (h *AuthUserMobileHandlerImpl) UserLogin(c fiber.Ctx) error {
	req := AuthLoginMobileRequest{}
...
func (h *AuthUserMobileHandlerImpl) Profile(c fiber.Ctx) error {
