...
type StoryMobileHandlerImpl struct {
	ss StoryMobileService
	ws *websocket.WebSocketManager
}

func NewStoryMobileHandlerImpl(dbpool *sqlx.DB, rdb *redis.Client, ws *websocket.WebSocketManager) *StoryMobileHandlerImpl {
	return &StoryMobileHandlerImpl{
		ss: NewStoryMobileServiceImpl(dbpool, rdb),
		ws: ws,
	}
}

func (h *StoryMobileHandlerImpl) Create(c fiber.Ctx) error {
...
func (h *StoryMobileHandlerImpl) Show(c fiber.Ctx) error {
...
func (h *StoryMobileHandlerImpl) Delete(c fiber.Ctx) error {
	uCtx, ok := c.Locals("UserContext").(share.UserContext)
	if !ok {
		return c.Status(fiber.StatusUnauthorized).JSON(
			response.NewResponseError("Unauthorized", constants.Generic_invalid, errors.New("user context not found")),
