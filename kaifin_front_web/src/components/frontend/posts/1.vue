<template>
  ...
  <div class="code-wrap" v-if="showCode">
    ...
    <!-- topic  -->
    <div class="topic-row" v-if="topics.length">
...
import Emoji from "../../../assets/animate/cool.svg"
import Cat from "../../../assets/animate/cat.svg"
import Bird from "../../../assets/animate/bird.svg"
import Succ from "../../../assets/animate/succ.svg"
import Lead from "../../../assets/animate/lead.svg"
...
// Mock data — ជំនួសដោយ API ពិតពេលក្រោយ
...
// State សម្រាប់ interactive photo positioning
const photoScale = ref(1)
const photoOffsetX = ref(0)   // offset បន្ថែមពី center (in canvas pixels)
const photoOffsetY = ref(0)
const baseScale = ref(1)      // scale គោល (cover-fit) គណនាដោយស្វ័យប្រវត្តិ
...
async function drawComposite() {
  ...
  // បំពេញ background ពណ៌ស មុន ដើម្បីកុំឲ្យមានចន្លោះខ្មៅ/ថ្លា ពេលរូបភាព user តូចជាង canvas
...
// ត្រូវ redraw រាល់ពេល scale/offset ប្តូរ
watch([photoScale, photoOffsetX, photoOffsetY], () => {
  drawComposite()
})

// === Drag handlers លើ canvas ===
...
function onCanvasPointerMove(e) {
  ...
  const scaleFactor = canvas.width / rect.width   // adjust ព្រោះ canvas display size ≠ actual pixel size
...
async function applyTemplate() {
  ...
  canvas.toBlob(blob => {
    ...
    photoOffsetX.value = 0
    photoOffsetY.value = 0
    frameNaturalWidth = 0   // reset ដើម្បីឲ្យ frame ត្រូវ reload សម្រាប់ template ថ្មីលើកក្រោយ
...
// function handleTemplateSelect() {
//   showPictureMenu.value = false
//   // TODO: logic សម្រាប់ Template
//   console.log('Template selected — logic not yet implemented')
// }

// Mock data សម្រាប់ Animated tab (design preview មុននឹងភ្ជាប់ API ពិត)
// const mockAnimatedPacks = ref([
//   {
//     id: 1,
...
//   // mock sticker ខាងក្នុង pack (ព្រោះមិនទាន់មាន API ពិត)
//   // នៅដំណាក់កាលនេះប្រើ sticker ដដែលពី stickers.value ជា placeholder
// }
...
// pack list សម្រាប់ tab 'animated' ប៉ុណ្ណោះ
// const currentPackList = computed(() => {
//   return mockAnimatedPacks.value
// })
...
async function onStickerFilePicked(e) {
  ...
  try {
    ...
    // formData.append('trigger_code', '')  // optional, can be delete stickers
    formData.append('file', file)          // it c.FormFile("file")
...
function addEmoji(e) {
    console.log('addEmoji called', e)
  postText.value += e
  showEmoji.value = false
  nextTick(() => textareaRef.value?.focus())
}

// File input images sticker 
...
// Func remove images on composer
function removeImage(i) {
  URL.revokeObjectURL(images.value[i].url)
  images.value.splice(i, 1)
}

// Confirm link composer
...
const stickerFilterTab = ref('all')   // 'all' | 'animated' | 'mine'
