...
type StoryReactionMobileHandlerImpl struct {
	srs StoryReactionMobileService
}

func NewStoryReactionMobileHandlerImpl(dbpool *sqlx.DB) *StoryReactionMobileHandlerImpl {
	return &StoryReactionMobileHandlerImpl{
		srs: NewStoryReactionServiceImpl(dbpool),
	}
}

func (h *StoryReactionMobileHandlerImpl) React(c fiber.Ctx) error {
...
func (h *StoryReactionMobileHandlerImpl) Show(c fiber.Ctx) error {
...
func (h *StoryReactionMobileHandlerImpl) Delete(c fiber.Ctx) error {
	uCtx, ok := c.Locals("UserContext").(share.UserContext)
	if !ok {
		return c.Status(fiber.StatusUnauthorized).JSON(
			response.NewResponseError("Unauthorized", constants.Generic_invalid, errors.New("user context not found")),
