...
type LikesMobileHandlerImpl struct {
	ls *LikesMobileServiceImpl
}

func NewLikesHandlerImpl(dbpool *sqlx.DB, rdb *redis.Client) *LikesMobileHandlerImpl {
	return &LikesMobileHandlerImpl{
		ls: NewLikesMobileServiceImpl(dbpool, rdb),
	}
}

func (h *LikesMobileHandlerImpl) Create(c fiber.Ctx) error {
...
func (h *LikesMobileHandlerImpl) Show(c fiber.Ctx) error {
	var uCtx *share.UserContext
	if ctx, ok := c.Locals("UserContext").(share.UserContext); ok {
		uCtx = &ctx
	}

	var req ShowLikesMobileRequest
	v := utls.NewValidator()
	if err := req.bind(c, v); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(
