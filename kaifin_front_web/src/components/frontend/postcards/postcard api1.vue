...
async function loadPosts(page) {
  ...
  try {
    ...
    // Fetch each post's like summary in the background so pagination
    // doesn't wait on N extra requests.
    mapped.forEach((post) => syncLikes(post))
  } catch (e) {
...
async function syncLikes(post) {
  try {
    ...
    post.likeCount = data.total ?? data.Total ?? post.likeCount
    post.isLiked = data.liked_by_me ?? data.LikedByMe ?? post.isLiked

    const myId = getCurrentUserId()
    const likeList = data.likes ?? data.Likes ?? []
    if (myId != null && Array.isArray(likeList)) {
      const mine = likeList.find((l) => l.user_id === myId || l.UserID === myId)
      post.reaction = mine ? mine.reaction_type ?? mine.Type ?? null : null
    }
  } catch (e) {
